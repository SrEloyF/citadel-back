...
class BannerService extends BaseService {
  ...
  async _resolveImage(data, file, previousUrl = null, id_imagen = null) {
    if (file) {
      const uploadedUrl = await storageService.upload(file, 'banners');
      if (previousUrl) {
  ...
  async delete(id) {
    ...
    try {
      const banner = await this.model.findByPk(id, { transaction: t });
      if (!banner) throw new NotFoundError('Banner no encontrado');
      ...
      if (!usedElsewhere && banner.url_img) {
        urlToDelete = banner.url_img;
      }
  ...
  async deleteExpired() {
    ...
    for (const banner of expiredBanners) {
      await this.delete(banner.id_imagen);
    }

    return { deletedCount: expiredBanners.length };
  }
}
